...
int Buttons::RenderText(string text, int x, int y, int size, SDL_Color color, int offset) {
	if (this->highlight == true) {
		SDL_SetRenderDrawColor(this->renderer, this->color[0], this->color[1], this->color[2], 255);
        SDL_RenderDrawRect(this->renderer, &d_rect);
	}
    int d_x = (x - (size / 2));
    int d_y = (y - (size / 2));
...
void Buttons::SetFont(string font, string location) {
